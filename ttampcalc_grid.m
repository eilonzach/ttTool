% produces structure "G" which contains the paths and amplitude information
% for the grid (can be over any grid, but you should be doing this for a
% grid over the US, so as to build up a predicted US amplitude map)
% 
% produces "glats", "glons", "gamps", which have amplitude at pts on grid
%% prelims
pathfile   = strcat(evdir,'path_grid_',Phase);
ofile_grid = sprintf('%s%samplitudes_grid',evdir,Phase);
if justplot==0
fprintf('\nComputing grid amps for phase %s\n',Phase);
%% GRID details 
fid = fopen(pathfile,'r');
ngrid = cell2mat(textscan(fid,'#NSTA: %f\n',1));
...
tic
for ig = 1:ngrid
if mod(ig-1,round(ngrid/20)) < 1; fprintf('%g%% done...\n',(100/20)*round(20*ig/ngrid)); end % progress report
%% read in sta info
...
G(ig).sta = ig; %will need to change this to char(stas(is)) when we have names
G(ig).slat = sedata(4);
G(ig).slon = sedata(5);
glats(ig) = G(ig).slat;
glons(ig) = G(ig).slon;
...
%takeoff (inc) and station incidence (sinc) angle
if psvsh == 1
inc = asind(rayp*Vpv0/(Re-edep)); % as Re-edep is radius of event
sinc = asind(rayp*Vpv(end)/Re); % angle at the surface
elseif psvsh == 2
inc = asind(rayp*Vsv0/(Re-edep)); % as Re-edep is radius of event
sinc = asind(rayp*Vsv(end)/Re); % angle at the surface
elseif psvsh == 3
inc = asind(rayp*Vsh0/(Re-edep)); % as Re-edep is radius of event
sinc = asind(rayp*Vsh(end)/Re); % angle at the surface
end
faz = azimuth(elat,elon,G(ig).slat,G(ig).slon);
baz = azimuth(G(ig).slat,G(ig).slon,elat,elon);
%% calc sections
sect = struct([]);
...
%% Reflection Transmission coefficients - use ENERGY coefficients
% Transmission
...
%% Diffraction effect...
if length(diffr.r)>0
    if any(diff(diffr.gcd)>5) % if true, there are gaps in diffracted portions - this is not an important diffracted phase... IGNORE
    AD = 1;
    else
    % Now find diffractive ray parameter (have to find ray type and radius of diffraction.
    if     abs(unique(rpath.rayt(indd(1))))==1
        pdlim = Rb(Rb==unique(diffr.r))/linterp(R,Vsh,diffr.r(1));
    elseif abs(unique(rpath.rayt(indd(1))))==2
        pdlim = Rb(Rb==unique(diffr.r))/linterp(R,Vpv,diffr.r(1));
    elseif abs(unique(rpath.rayt(indd(1))))==3
        pdlim = Rb(Rb==unique(diffr.r))/linterp(R,Vsv,diffr.r(1));
    end
    % from this find lambda
    lambda = pdlim^(1/3); % FUDGE here - see Aki & Richards p.457.
    diffgcds = rpath.gcd(rpath.r == diffr.r(1));
    diffr.Dgcd = diffgcds(end) - diffgcds(1);
    AD = exp(-w0.^(1/3).*lambda.*sin(pi/3).*diffr.Dgcd); % NOT 100% SURE IF THIS DELTA SHOULD BE IN DEGREES (AS IT IS) OR RADIANS
    end
else
AD = 1;
end
...
% Starting amp. and geometrical spreading fn
if psvsh == 1
    RR = (Re-edep)*Re*sqrt(cosd(inc)*cosd(sinc)*sind(rpath.gcd(end))*abs(dDdp)/(1000*d2r(rayp)))/Vpv0;
    A0 = 1./(4*pi*sqrt(rho0*rho(end)*Vpv0*Vpv(end))*Vpv0^2*RR);
elseif psvsh == 2
    RR = (Re-edep)*Re*sqrt(cosd(inc)*cosd(sinc)*sind(rpath.gcd(end))*abs(dDdp)/(1000*d2r(rayp)))/Vsv0;
    A0 = 1./(4*pi*sqrt(rho0*rho(end)*Vsv0*Vsv(end))*Vsv0^2*RR);
elseif psvsh == 3
    RR = (Re-edep)*Re*sqrt(cosd(inc)*cosd(sinc)*sind(rpath.gcd(end))*abs(dDdp)/(1000*d2r(rayp)))/Vsh0;
    A0 = 1./(4*pi*sqrt(rho0*rho(end)*Vsh0*Vsh(end))*Vsh0^2*RR);
end
A = A0*Rpat(psvsh)*AQ*ATRW*AD; % final magnitude
A = A0*Rpat(psvsh)*AQ*ATRW*AD; % final magnitude
% if abs(A)>Acutoff; samps(ip) = Acutoff*sign(A); else samps(ip) = A; end
...
phs.bounce = bounce;
phs.diffr = diffr;
G(ig).phase = char(phase{1});
G(ig).rays(ia) = phs;
end % arrivals loop

gamps(ig) = G(ig).rays(1).A;
% %% fudge caustic...
% if length(G(is).rays) > 1
%     if G(ig).rays(2).tt - G(ig).rays(1).tt < 15
%         samps(ig) = samps(ig) + G(ig).rays(2).A;
%     end
% end
end % grid loop
